...
func loadConfig(path string) (*Config, error) {
	...
	if cfg.Security.WGPrivateKeyPath == "" {
		cfg.Security.WGPrivateKeyPath = "/etc/nerve/wg-private.key"
	}
...
func (cfg *Config) ensureIdentity(cfgPath string) error {
	...
	fmt.Printf("nerved: generated node ID: %s\n", cfg.Node.ID)

	// Rewrite the config file so the generated ID persists across reboots.
	var buf bytes.Buffer
	if err := toml.NewEncoder(&buf).Encode(cfg); err != nil {
		fmt.Printf("nerved: warning: could not encode config: %v\n", err)
		return nil
	}
	if err := os.WriteFile(cfgPath, buf.Bytes(), 0o640); err != nil {
		fmt.Printf("nerved: warning: could not persist node ID: %v\n", err)
	}
	return nil
}
