...
func init() {
	...
	// nice=-10 puts nerved above all normal user processes (which start at 0)
	...
	if err := syscall.Setpriority(syscall.PRIO_PROCESS, 0, -10); err != nil {
		log.Printf("nerved: could not set nice=-10: %v", err)
	}
	// Protect ourselves from OOM kills — nerved must survive memory pressure.
	if err := os.WriteFile("/proc/self/oom_score_adj", []byte("-900\n"), 0o644); err != nil {
		log.Printf("nerved: could not set oom_score_adj: %v", err)
	}
}
