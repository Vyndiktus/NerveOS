...
// DeriveNerveIP returns a deterministic /32 IP in 10.42.0.0/16
...
func DeriveNerveIP(nodeID string) (net.IP, error) {
	if len(nodeID) < 4 {
		return nil, fmt.Errorf("nerveip: node ID too short: %q", nodeID)
	}
	b, err := hex.DecodeString(nodeID[:4])
	if err != nil {
		return nil, fmt.Errorf("nerveip: decode node ID: %w", err)
	}
...
// NerveIPNet returns the /32 net.IPNet for a given hive IP.
func NerveIPNet(ip net.IP) net.IPNet {
	return net.IPNet{
		IP:   ip,
