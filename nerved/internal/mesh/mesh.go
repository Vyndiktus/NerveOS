...
type Peer struct {
	...
	NerveIP    net.IP    // peer's IP in the hive subnet
	AllowedIP net.IPNet // /32 of NerveIP
}
...
func (m *Manager) Start() error {
	...
	hiveIP, err := DeriveNerveIP(m.cfg.NodeID)
	if err != nil {
...
// NerveIP returns this node's IP in the hive subnet.
func (m *Manager) NerveIP() net.IP { return m.hiveIP }
...
func (m *Manager) AddPeer(p *Peer) error {
	...
	log.Printf("mesh: peer added id=%s endpoint=%s hive-ip=%s", p.ID, p.Endpoint, p.NerveIP)
	return nil
}
