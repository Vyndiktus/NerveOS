#!/usr/bin/env python3
"""Restore original hci_uart.ko from Buildroot rootfs to device, then reboot."""
...
result = subprocess.run(
    ["wsl", "-d", "Debian", "-u", "root", "--",
     "bash", "-c", "cat /opt/hci_uart_orig.ko"],
    capture_output=True
)
ko_data = result.stdout
print(f"hci_uart.ko (original): {len(ko_data)} bytes")
...
# Reboot to fastboot for clean cold test
print("\nRebooting to fastboot for clean cold test...")
s.write(b'/sbin/reboot-bootloader\n')
read_for(s, 5)
s.close()
print("Done — device rebooting to fastboot")
