...
import (
	...
	"NerveOS/hived/internal/transport"
)
...
func (d *Manager) addStaticPeer(entry string) error {
	...
	provisionalIP := provisionalHiveIP(pubKey)

	peer := &mesh.Peer{
		ID:        pubKey[:8],
		PublicKey: pubKey,
		Endpoint:  endpoint,
		HiveIP:    provisionalIP,
	}
	return d.meshMgr.AddPeer(peer)
}

// provisionalHiveIP derives a hive IP from a WireGuard public key string
// for use before the peer has announced its canonical hive IP.
func provisionalHiveIP(pubKey string) net.IP {
	sum := 0
